...
/*
 * 功能: 读取带范围校验的整数输入
 * 入参: prompt - 提示文本, minValue - 最小值, maxValue - 最大值
 * 返回: 用户输入的有效整数
 * 异常: 空输入/非数字/超出范围时分别提示并重新输入
 */
...
/*
 * 功能: 读取单个字符输入并校验合法性
 * 入参: prompt - 提示文本, valid - 合法字符集字符串
 * 返回: 用户输入的合法字符
 * 异常: 输入字符不在valid集合中则提示后重新输入
 */
...
/*
 * 功能: 获取当前日期时间字符串(YYYY-MM-DD HH:MM:SS)
 * 入参: buf - 输出缓冲区, size - 缓冲区大小(至少20字节)
 */
...
/*
 * 功能: 校验日期字符串是否合法
 * 入参: date - 格式为YYYY-MM-DD的日期字符串
 * 返回: 合法返回true, 否则返回false
 * 异常: 长度不对/分隔符位置不对/含非数字字符/年月日超出范围/闰年2月29天
 */
...
/*
 * 功能: 将日期字符串转换为从2000-01-01起的天数
 * 入参: date - 格式为YYYY-MM-DD的日期字符串
 * 返回: 从2000-01-01起至该日期的总天数
 */
 ...
 * 功能: 按ID在租车记录链表中查找
 * 入参: id - 租车记录编号
 * 返回: 找到返回RentRecord指针，未找到返回nullptr
 */
...
/*
 * 功能: 将用户链表二进制写入renter.dat
 * 格式: [count:4B] [Renter×N]
 */
void saveRenters() {
...
/*
 * 功能: 将租车记录链表二进制写入rent.dat
 * 格式: [count:4B] [RentRecord×N]
 */
void saveRents() {
...
/*
 * 功能: 将用户名密码二进制写入pwd.dat
 * 格式: [username:32B] [password_encrypted:64B]
 */
void savePassword() {
...
/*
 * 功能: 从renter.dat二进制文件加载用户链表
 * 异常: 数据文件缺失或count异常则安全跳过
 */
void loadRenters() {
...
/*
 * 功能: 从pwd.dat二进制文件加载用户名和密码
 * 异常: 文件不存在则保持passwordReady=false(首次使用)
 */
void loadPassword() {
...
bool checkPassword(const char* plain) {
    ...
    return memcmp(enc, passwordStore, sizeof(passwordStore)) == 0; // 对比64是否相同
}
/*
 * 功能: 设置新密码(长度校验+PWD_MIN~PWD_MAX, XOR加密后存储)
 * 入参: plain - 新密码明文
 * 返回: 长度符合要求且保存成功返回true
 */
 ...
 * 功能: 从车辆链表中删除指定ID的车辆
 * 入参: id - 要删除的车辆编号
 * 返回: 找到并删除成功返回true, 未找到返回false
 * 异常: 删除后链表的prev->next重链接, 头节点特殊处理, 内存通过delete释放
 */
...
/*
 * 功能: 办理退车结算(计算费用与退款, 更新车辆状态)
 * 入参: rentId - 租车记录ID, returnDate - 实际归还日期, totalFee - 输出总费用, refund - 输出退款金额
 * 返回: 退车成功返回true
 * 异常: 超期天数按2倍日租金计费, 退款=押金-总费用(最低0)
 */
bool returnRent(int rentId, const char* returnDate, double& totalFee, double& refund) {
    ...
    refund = rec->deposit; // 退款=全额退还押金, 租车费用单独支付
...
/*
 * 功能: 打印退车费用收据
 * 入参: r - 租车记录, days - 租用天数, total - 总费用, refund - 退款金额
 */
...
/*
 * 功能: 打印用户表头(ID 姓名 年龄 性 电话 驾照 身份证 驾龄 次数)
 */
...
/*
 * 功能: 打印租车记录表头(ID 车 人 租车日期 预计归还 实际归还 押金 日租金 总费用 状态)
 */
...
/*
 * 功能: 按类型分组统计车辆数量
 * 入参: labels - 输出类别名称数组, values - 输出各类数量, max - 最大类别数
 * 返回: 实际类别数
 */
...
int countVehiclesByColor(char labels[][32], int values[], int max) {
    ...
    while (cur) {
        ...
        if (found >= 0) values[found]++;
        else if (n < max) { strncpy(labels[n], cur->data.color, 31); values[n] = 1; n++; } // 比较color
        cur = cur->next;
    }
    return n;
}
...
/*
 * 功能: 按驾龄分段统计用户数量
 * 入参: values - [0]≤3年, [1]4-10年, [2]10年以上
 */
...
/*
 * 功能: 打印系统概览统计(总车辆/可租/已租/总用户/记录/进行中/总收入)
 */
...
void showBanner() {
    ...
    printf("\n汽车租赁管理系统 by RayChen(Rayawa) %s\n", now);
}
// 主界面，打印选项之后switch
bool mainMenu() {
    showBanner(); // 每轮循环实时刷新日期时间
...
void runApp() {
    ...
    while (mainMenu()) {} // 主程序循环(showBanner已在mainMenu内每轮调用)
